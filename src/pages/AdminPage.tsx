...
import { TarifeDialog } from '@/components/dialogs/TarifeDialog';
import { TarifeViewDialog } from '@/components/dialogs/TarifeViewDialog';
...
const TARIFE_FIELDS = [
  { key: 'name', label: 'Tarifname', type: 'string/text' },
  { key: 'monatspreis', label: 'Monatspreis (€)', type: 'number' },
];
const MITGLIEDSCHAFTEN_FIELDS = [
  { key: 'mitglied', label: 'Mitglied', type: 'applookup/select', targetEntity: 'mitglieder', targetAppId: 'MITGLIEDER', displayField: 'vorname' },
  { key: 'tarif', label: 'Tarif', type: 'applookup/select', targetEntity: 'tarife', targetAppId: 'TARIFE', displayField: 'name' },
  { key: 'zahlweise', label: 'Zahlweise', type: 'lookup/radio', options: [{ key: 'monatlich', label: 'Monatlich' }, { key: 'jaehrlich', label: 'Jährlich' }, { key: 'einmalig', label: 'Einmalig' }] },
  { key: 'laufzeit_monate', label: 'Laufzeit (Monate)', type: 'number' },
  { key: 'rabatt_prozent', label: 'Rabatt (%)', type: 'number' },
  { key: 'beitrag', label: 'Beitrag (€, berechnet)', type: 'number' },
  { key: 'eintritt', label: 'Eintrittsdatum', type: 'date/date' },
  { key: 'aktiv', label: 'Mitgliedschaft aktiv', type: 'bool' },
];

const ENTITY_TABS = [
  { key: 'mitglieder', label: 'Mitglieder', pascal: 'Mitglieder' },
  { key: 'tarife', label: 'Tarife', pascal: 'Tarife' },
  { key: 'mitgliedschaften', label: 'Mitgliedschaften', pascal: 'Mitgliedschaften' },
] as const;
...
export default function AdminPage() {
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      ...
      case 'tarife': return (data as any).tarife as Tarife[] ?? [];
      case 'mitgliedschaften': return (data as any).mitgliedschaften as Mitgliedschaften[] ?? [];
      default: return [];
    }
  }, [data]);
  ...
  const getLookupLists = useCallback((entity: EntityKey) => {
    ...
    switch (entity) {
      case 'mitgliedschaften':
        ...
        lists.tarifeList = (data as any).tarife ?? [];
        break;
    }
    return lists;
  }, [data]);
  ...
  const getApplookupDisplay = useCallback((entity: EntityKey, fieldKey: string, url?: unknown) => {
    ...
    if (entity === 'mitgliedschaften' && fieldKey === 'mitglied') {
      const match = (lists.mitgliederList ?? []).find((r: any) => r.record_id === id);
      return match?.fields.vorname ?? '—';
    }
    if (entity === 'mitgliedschaften' && fieldKey === 'tarif') {
      const match = (lists.tarifeList ?? []).find((r: any) => r.record_id === id);
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    switch (entity) {
      ...
      case 'tarife': return TARIFE_FIELDS;
      case 'mitgliedschaften': return MITGLIEDSCHAFTEN_FIELDS;
      default: return [];
    }
  }, []);
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      ...
      case 'tarife': return {
        create: (fields: any) => LivingAppsService.createTarifeEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateTarifeEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteTarifeEntry(id),
      };
      case 'mitgliedschaften': return {
        create: (fields: any) => LivingAppsService.createMitgliedschaftenEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateMitgliedschaftenEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteMitgliedschaftenEntry(id),
      };
      default: return null;
    }
  }, []);
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'tarife' || dialogState?.entity === 'tarife') && (
        <TarifeDialog
          open={createEntity === 'tarife' || dialogState?.entity === 'tarife'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'tarife' ? handleUpdate : (fields: any) => handleCreate('tarife', fields)}
          defaultValues={dialogState?.entity === 'tarife' ? dialogState.record?.fields : undefined}
          enablePhotoScan={AI_PHOTO_SCAN['Tarife']}
          enablePhotoLocation={AI_PHOTO_LOCATION['Tarife']}
        />
      )}
      {(createEntity === 'mitgliedschaften' || dialogState?.entity === 'mitgliedschaften') && (
        <MitgliedschaftenDialog
          open={createEntity === 'mitgliedschaften' || dialogState?.entity === 'mitgliedschaften'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'mitgliedschaften' ? handleUpdate : (fields: any) => handleCreate('mitgliedschaften', fields)}
          defaultValues={dialogState?.entity === 'mitgliedschaften' ? dialogState.record?.fields : undefined}
          mitgliederList={(data as any).mitglieder ?? []}
          tarifeList={(data as any).tarife ?? []}
      ...
      {viewState?.entity === 'tarife' && (
        <TarifeViewDialog
          open={viewState?.entity === 'tarife'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'tarife', record: r }); }}
        />
      )}
      {viewState?.entity === 'mitgliedschaften' && (
        <MitgliedschaftenViewDialog
          open={viewState?.entity === 'mitgliedschaften'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'mitgliedschaften', record: r }); }}
          mitgliederList={(data as any).mitglieder ?? []}
          tarifeList={(data as any).tarife ?? []}
        />
      )}
