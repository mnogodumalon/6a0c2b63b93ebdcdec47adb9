...
import type { Mitglieder, Tarife, Mitgliedschaften, CreateMitglieder, CreateTarife, CreateMitgliedschaften } from '@/types/app';
...
export class LivingAppsService {
  ...
  // --- TARIFE ---
  static async getTarife(): Promise<Tarife[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.TARIFE}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Tarife[];
    return enrichLookupFields(records, 'tarife');
  }
  static async getTarifeEntry(id: string): Promise<Tarife | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.TARIFE}/records/${id}`);
    const record = { record_id: data.id, ...data } as Tarife;
    return enrichLookupFields([record], 'tarife')[0];
  }
  static async createTarifeEntry(fields: CreateTarife) {
    return callApi('POST', `/apps/${APP_IDS.TARIFE}/records`, { fields: cleanFieldsForApi(fields as any, 'tarife') });
  }
  static async updateTarifeEntry(id: string, fields: Partial<CreateTarife>) {
    return callApi('PATCH', `/apps/${APP_IDS.TARIFE}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'tarife') });
  }
  static async deleteTarifeEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.TARIFE}/records/${id}`);
  }

  // --- MITGLIEDSCHAFTEN ---
  static async getMitgliedschaften(): Promise<Mitgliedschaften[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.MITGLIEDSCHAFTEN}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Mitgliedschaften[];
    return enrichLookupFields(records, 'mitgliedschaften');
  }
  static async getMitgliedschaftenEntry(id: string): Promise<Mitgliedschaften | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.MITGLIEDSCHAFTEN}/records/${id}`);
    const record = { record_id: data.id, ...data } as Mitgliedschaften;
    return enrichLookupFields([record], 'mitgliedschaften')[0];
  }
  static async createMitgliedschaftenEntry(fields: CreateMitgliedschaften) {
    return callApi('POST', `/apps/${APP_IDS.MITGLIEDSCHAFTEN}/records`, { fields: cleanFieldsForApi(fields as any, 'mitgliedschaften') });
  }
  static async updateMitgliedschaftenEntry(id: string, fields: Partial<CreateMitgliedschaften>) {
    return callApi('PATCH', `/apps/${APP_IDS.MITGLIEDSCHAFTEN}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'mitgliedschaften') });
  }
  static async deleteMitgliedschaftenEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.MITGLIEDSCHAFTEN}/records/${id}`);
  }
